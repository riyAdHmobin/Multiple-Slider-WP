(function ( $, window, document ) {
	...
	$(document).on('cmb2_add_row', function(e, added_row) {
		// Selector Control
		added_row.find( '.gamipress-selector select' ).each(function() {
			gamipress_select2_destroy( $(this), false );
			gamipress_selector( $(this) );
		});

		// Post Selector Control
		added_row.find( '.gamipress-post-selector select' ).each(function() {
			gamipress_select2_destroy( $(this), true );
			gamipress_post_selector( $(this) );
		});

		// User Selector Control
		added_row.find( '.gamipress-user-selector select' ).each(function() {
			gamipress_select2_destroy( $(this), true );
			gamipress_user_selector( $(this) );
		});
	});
